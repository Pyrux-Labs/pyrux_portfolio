...
const ctaColorTokens: Record<PlanColor, { text: string; border: string; bg: string; glow: string }> = {
	growth:   { text: "text-[var(--pkg-growth)]",    border: "border-[var(--pkg-growth)]",    bg: "bg-[var(--pkg-growth-soft)]",    glow: "shadow-[0_0_24px_var(--pkg-growth-soft)]" },
	pro:      { text: "text-coral",                   border: "border-coral",                   bg: "bg-[var(--color-coral-soft-bg)]", glow: "shadow-[0_0_24px_var(--shadow-coral-soft)]" },
	business: { text: "text-[var(--pkg-business)]",  border: "border-[var(--pkg-business)]",  bg: "bg-[var(--pkg-business-soft)]",  glow: "shadow-[0_0_24px_var(--pkg-business-soft)]" },
	custom:   { text: "text-[var(--pkg-custom)]",    border: "border-[var(--pkg-custom)]",    bg: "bg-[var(--pkg-custom-soft)]",    glow: "shadow-[0_0_24px_var(--pkg-custom-soft)]" },
};
...
export default function PreciosPageClient() {
	...
	const visiblePackages = packages.filter((p) => p.category === selectedCategory);
	...
	useEffect(() => {
		...
		children[selectedPkg]?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "start" });
			...
			<main className="relative z-1 max-w-content mx-auto min-h-screen flex flex-col px-4 pt-20 pb-8 min-[481px]:px-6 min-[481px]:pb-10">
				...
				<Section id="paquetes" className="mb-6" title="Nuestros paquetes">
						...
						<div className="md:hidden">
							<div
								...
								{visiblePackages.map((pkg, idx) => (
									<div key={`${pkg.category}-${pkg.number}`} className="snap-start shrink-0 w-[85vw]">
										<PackageCard
											pkg={pkg}
					...
					<motion.div
						...
						className={
							selectedCategory === "personalizado"
								? "flex justify-center"
								: "hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-5 items-stretch"
						}
				...
				{selectedCategory !== "personalizado" && activePkg && (
					<motion.div
						...
						className={`flex items-center justify-between gap-4 mb-10 px-5 py-4 rounded-xl border backdrop-blur-sm ${ctaColorTokens[activePkg.planColor].border} ${ctaColorTokens[activePkg.planColor].bg}`}>
							<div className="flex flex-col min-[481px]:flex-row min-[481px]:items-center gap-1 min-[481px]:gap-3">
								<span className="text-[0.8rem] text-muted">Plan seleccionado</span>
							...
							<a
								...
								className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-[0.85rem] font-semibold transition-all duration-200 shrink-0 no-underline hover:brightness-110 hover:scale-[1.02]
					...
					<Section
						...
						titleNode={
							<>
								...
								<span className={ctaColorTokens[activePkg.planColor].text}>
									{activePkg.name}
								</span>
							</>
						}>
