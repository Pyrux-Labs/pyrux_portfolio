...
export default function CreatorContent({ creatorId }: CreatorContentProps) {
	const t = useTranslations("CreatorPage");
	const { locale } = useLocale();
	const { copy } = useCopyToClipboard();
	const creator = getCreatorById(creatorId, locale);
	const creatorProjects = useMemo(() => getProjectsByCreator(creatorId, locale), [creatorId, locale]);
	...
	if (!creator) {
		return (
			<main className="max-w-content mx-auto flex-1 flex flex-col items-center justify-center px-4">
				<h1 className="font-display text-2xl font-bold text-primary mb-4">{t("notFoundTitle")}</h1>
				<Link href="/" className="text-coral no-underline hover:text-cyan transition-colors">
					{t("backToHomeAlt")}
				</Link>
			</main>
		);
				...
				<motion.div
					...
					{creatorProjects.map((project) => (
						<motion.div
							...
							<h3 className="font-display text-[1.05rem] font-semibold text-primary">{project.title}</h3>
							...
							<div className="flex items-center justify-between">
								...
								{project.featured && (
									<Badge label={t("featured")} variant="coral" className="text-[0.65rem]" />
								)}
							</div>
						</motion.div>
					))}
